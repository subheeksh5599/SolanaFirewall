          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {!stats && isOwner !== false && (
              <motion.div
                ...
                className="lg:col-span-3 rounded-2xl border border-blue-500/30 bg-blue-500/5 p-8 text-center"
              >
                <h3 className="text-xl font-medium mb-3">Initialize the Firewall</h3>
                <p className="text-neutral-400 mb-5 text-sm">This creates the on-chain FirewallState PDA. Only needs to be called once.</p>
            ...
            <div className="lg:col-span-2 border border-white/10 rounded-2xl p-6">
              ...
              <div className="flex flex-col gap-3">
                <input
                  ...
                  className="bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-white/30"
                />
                <div className="flex gap-3">
                  <input
                    type="number" step="0.001" placeholder="Amount in SOL"
                    value={amount} onChange={e => setAmount(e.target.value)}
                    className="flex-1 bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-white/30"
                  />
                  <button
                    onClick={() => executePayment(recipient, parseFloat(amount))}
                    disabled={loading || !recipient}
                    className="bg-white text-black rounded-xl px-6 py-2.5 font-medium hover:bg-neutral-200 disabled:opacity-40 transition-colors"
                  >
                    {loading ? "Sending..." : "Pay"}
                  </button>
                </div>
                <div className="flex gap-2">
                  {[0.001, 0.01, 100, 15000].map(a => (
                    <button key={a} onClick={() => setAmount(String(a))} className="text-xs bg-white/5 rounded-lg px-3 py-1.5 text-neutral-400 hover:text-white transition-colors">
                      {a} SOL
                    </button>
                  ))}
                </div>
            ...
            <div className="border border-white/10 rounded-2xl p-6 flex flex-col">
              ...
              <button
                onClick={() => simulate(recipient || "11111111111111111111111111111111", parseFloat(amount))}
                disabled={loading}
                className="bg-white/10 text-white rounded-xl px-6 py-2.5 font-medium hover:bg-white/20 disabled:opacity-40 transition-colors mb-4"
              >
                Simulate Payment
              </button>
              {result && (
            ...
            <div className="lg:col-span-3 border border-white/10 rounded-2xl p-6">
              ...
              <p className="text-sm text-neutral-400 mb-4">
                The firewall enforces policies on-chain. Try sending 15,000 SOL to see it get blocked — the Solana runtime itself prevents the transfer.
              </p>
              <div className="flex flex-wrap gap-2">
                <span className="text-xs bg-white/5 rounded-lg px-3 py-1.5 text-neutral-400">
                  PDA: {firewallPda.toBase58().slice(0, 12)}...
                </span>
              </div>
            </div>
          </div>
